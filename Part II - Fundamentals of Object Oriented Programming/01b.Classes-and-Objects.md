...
* To declare pointers to class members - i.e. class member pointers the ::* operator is used. 

A data member needs to be associated with an object, for it to be allocated memory space and hence have an address, defining a pointer to it does not make any sense. Also, since the address of a static member is not associated with any particular object, normal pointers are used to access static data members.
...
class A
{
    ...
    void putData()
    {
        cout << "Private data member accessed using a pointer " << *nptr << "\n";
    }
