...
Private data members of a class are accessed using the member functions of that class, public data members can be accessed using the objects directly:
...
> Private (and protected) data members however can be acessed using class objects and dot operator inside a friend function.
...
Usually all the data items are placed in the private sections of a class, and all functions are public. In certain situations it may be necessary to require certain functions to be hidden e.g. deleting files, and therefore require restricted access. Such functions are placed in the private sections.

A private member function can only be called by another function which is a member of its class. Even an object cannot invoke a private function using the dot operator. 
