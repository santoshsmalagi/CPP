...
Object aObj
Pointer to object aObjPtr
Pointer to variable mPtr
Pointer to function mFunc

 

For a class A: 
A::\*m  ---- > pointer to a member m of class A  
&A::m ---- > address of member m in class A



#### Pointers to the Members of a Class
A pointer to the member of a class can be declared using the ::* operator.

The following is an invalid statement, and does not have any significance, because ```m``` needs to be associated with the class to which it belongs.
int \*ip = &m;     

int A::\*ip = &A::m;

Pointer ip can now be used to access the member m inside member functions (or friend functions). If ```a``` is an object of class A, then:
